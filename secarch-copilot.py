...
# Function to get user input for the application description and key information
def get_input():
...
st.set_page_config(
    page_title="Security Architect Copilot",
    page_icon=":shield:",
    layout="wide",
...
st.sidebar.header("How to use SecArch Copilot")
...
with st.sidebar:
    st.markdown(
        "Welcome to SecArch Copilot, an AI-powered tool designed to help teams produce better threat models for their applications."
    )
    st.markdown(
        "Threat modelling is a key activity in the secure@design phase, but is often poorly or inconsitently executed. SecArch Copilot aims to help our teams produce more comprehensive threat models by leveraging the power of Large Language Models (LLMs) to generate a threat list, attack tree and/or mitigating controls for an application based on the details provided."
    )
    st.markdown("""---""")
...
with st.sidebar:
    st.markdown(
        "Below is an example application description that you can use to test SecArch Copilot:"
    )
    st.markdown(
...
with st.sidebar:
    ...
    No, SecArch Copilot does not store your application description or other details. All entered data is deleted after you close the browser tab.
    """
    )
    st.markdown(
        """
    ...
    No, the threat models are not 100% accurate. SecArch Copilot uses GPT Large Language Models (LLMs) to generate its output. The GPT models are powerful, but they sometimes makes mistakes and are prone to 'hallucinations' (generating irrelevant or inaccurate content). Please use the output only as a starting point for identifying and addressing potential security risks in your applications.
    """
    )
    st.markdown(
        """
...
with st.expander("Threat Model", expanded=False):
    ...
    threat_model_submit_button = st.button(label="Generate test Threat Model")
